...
(function () {
  ...
  var CONFIG = {
    // Frames live in frames-1/, frames-2/, frames-3/, ... with 100 frames
    // per folder: frames-1 holds frame001–frame100, frames-2 holds
    // frame101–frame200, and so on.
    framesPerFolder: 100,
    framePrefix: "frame",
    frameDigits: 3,
    frameExtension: ".jpg",
    frameCount: 121, // total extracted frames
  ...
  function frameFolder(n) {
    var folderIndex = Math.ceil(n / CONFIG.framesPerFolder);
    return "frames-" + folderIndex + "/";
  }

  function frameUrl(n) {
    return frameFolder(n) + CONFIG.framePrefix + padFrameNumber(n) + CONFIG.frameExtension;
  }
  ...
  function preloadFrames() {
    for (var i = 0; i < CONFIG.frameCount; i++) {
      ...
      img.src = frameUrl(i + 1); // files are frames-1/frame001.jpg .. frames-2/frame121.jpg (1-indexed)
      frames[i] = img;
    }
  }
